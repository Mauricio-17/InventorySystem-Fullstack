    ...
    <a-drawer v-model:visible="visible" class="custom-class" style="color: red" title="EMPLEADO" placement="right"
        ...
        <a-form :model="object" name="basic" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" autocomplete="off"
            ...
            <a-form-item label="Tipo propietario" name="typeOwner">
                <a-select label="Tipo propietario" ref="select" v-model:value="object.typeOwner.id" style="width: 150px">
...
const submitObject = async () => {
    ...
    try {
        ...
        successNotification("Registro exitoso", `El propietario ${item.name.toUpperCase()} ha sido registrado.`);
        emit('update-list');
    } catch (e) {
        if (e.response) {
