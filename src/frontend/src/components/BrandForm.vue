...
const submitObject = async () => {
    ...
    try {
        let result = await addNewBrand(item);
        successNotification("Registro exitoso", `La marca ${item.name.toUpperCase()} ha sido registrado.`);
        emit('update-list');
    } catch (e) {
        if (e.response) {
            ...
            errorNotification("Ocurrió un error la marca :(", data.message);
        }
    } finally {
        hideDrawer();
        cleanObject();
